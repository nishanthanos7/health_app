...
export default function ClinicPage() {
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-[#0056b3] selection:text-white ${lang === 'ne' ? 'nepali' : ''}`}>
      {/* Top Bar */}
      <div className="bg-[#0056b3] text-white py-2 hidden md:block">
      ...
      {/* Navigation */}
      <nav className={`fixed top-0 md:top-8 w-full z-50 transition-all duration-300 ${scrolled ? 'md:top-0 bg-white/80 backdrop-blur-md border border-white/20 shadow-md py-3' : 'bg-transparent py-6'}`}>
        <div className="container mx-auto px-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <span className="font-[family-name:var(--font-display)] font-black text-2xl leading-tight text-slate-900">{lang === 'en' ? 'All Nepal Health' : 'अल नेपाल हेल्थ'}</span>
              <span className="text-[11px] uppercase tracking-[0.2em] text-[#0056b3] font-bold">{lang === 'en' ? 'Treatment Clinic' : 'ट्रिटमेन्ट क्लिनिक'}</span>
            </div>
          </div>

          <div className="hidden md:flex items-center gap-10">
            {navItems.map((item) => (
              <a key={item.name} href={item.href} className="text-sm font-bold text-slate-700 hover:text-[#0056b3] transition-colors relative group">
                {item.name}
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-[#0056b3] transition-all group-hover:w-full" />
              </a>
            ))}
            <button onClick={toggleLang} className="flex items-center gap-2 px-5 py-2.5 rounded-xl border-2 border-slate-200 text-sm font-bold hover:border-[#0056b3] hover:text-[#0056b3] transition-all bg-white">
              <Globe size={18} />
              {lang === 'en' ? 'नेपाली' : 'English'}
            </button>
            <a href="#contact" className="bg-[#0056b3] text-white px-7 py-3 rounded-xl text-sm font-black shadow-xl shadow-[#0056b3]/30 hover:bg-blue-700 transition-all active:scale-95 hover:-translate-y-0.5">
              {t.hero.cta}
            </a>
          </div>
        ...
        {isMenuOpen && (
          <motion.div initial={{ opacity: 0, x: '100%' }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: '100%' }} className="fixed inset-0 z-[60] bg-white md:hidden flex flex-col">
            ...
            <div className="flex-1 flex flex-col p-8 gap-8">
              {navItems.map((item) => (
                <a key={item.name} href={item.href} onClick={() => setIsMenuOpen(false)} className="text-3xl font-black text-slate-900">{item.name}</a>
              ))}
              <div className="mt-auto flex flex-col gap-4">
                <button onClick={toggleLang} className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl border-2 border-slate-200 text-xl font-bold text-[#0056b3]">
                  <Globe size={24} /> {lang === 'en' ? 'नेपाली' : 'English'}
                </button>
                <a href="#contact" onClick={() => setIsMenuOpen(false)} className="w-full bg-[#0056b3] text-white py-5 rounded-2xl text-center text-xl font-black shadow-xl shadow-[#0056b3]/20">{t.hero.cta}</a>
              </div>
            </div>
          </motion.div>
        )}
      ...
      <section id="home" className="relative min-h-screen flex items-center pt-32 pb-20 overflow-hidden bg-slate-50">
        <div className="absolute inset-0 -z-10">
          <div className="absolute top-0 right-0 w-[60%] h-[60%] bg-[#0056b3]/5 rounded-full blur-[120px]" />
          <div className="absolute bottom-0 left-0 w-[40%] h-[40%] bg-[#00a8e8]/10 rounded-full blur-[100px]" />
        </div>
        <div className="container mx-auto px-4 grid lg:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-[#0056b3]/10 text-[#0056b3] text-sm font-black uppercase tracking-widest mb-8">
              <Zap size={16} fill="currentColor" /> {t.hero.slogan}
            </div>
            <h1 className="text-5xl md:text-8xl font-black text-slate-900 leading-[1.05] mb-8">{t.hero.title}</h1>
            <p className="text-xl text-slate-600 mb-12 max-w-xl leading-relaxed font-medium">{t.hero.subtitle}</p>
            <div className="flex flex-wrap gap-5">
              <a href="#contact" className="bg-[#0056b3] text-white px-10 py-5 rounded-2xl font-black text-lg shadow-2xl shadow-[#0056b3]/30 hover:bg-blue-700 transition-all hover:-translate-y-1 flex items-center gap-3">
                {t.hero.cta} <ArrowRight size={20} />
              </a>
              <a href="#services" className="bg-white text-slate-900 px-10 py-5 rounded-2xl font-black text-lg border-2 border-slate-200 hover:border-[#0056b3] hover:text-[#0056b3] transition-all">{t.hero.secondaryCta}</a>
            </div>
          </motion.div>
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
            <div className="max-w-2xl">
              <div className="text-[#0056b3] font-black uppercase tracking-[0.3em] text-sm mb-4">{lang === 'en' ? 'What we offer' : 'हाम्रा सेवाहरू'}</div>
              <h2 className="text-4xl md:text-6xl font-black text-slate-900">{t.services.title}</h2>
            </div>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service, i) => (
              <motion.div
                ...
                className="p-10 rounded-[2.5rem] bg-slate-50 border-2 border-transparent hover:border-[#0056b3]/10 hover:bg-white hover:shadow-2xl transition-all group cursor-pointer relative overflow-hidden"
              >
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden">
              ...
              <div className="p-10">
                ...
                <a href="#contact" onClick={() => setSelectedService(null)} className="inline-flex items-center gap-3 bg-[#0056b3] text-white px-8 py-4 rounded-2xl font-black shadow-xl shadow-[#0056b3]/20 hover:bg-blue-700 transition-all">
                  {t.hero.cta} <ArrowRight size={20} />
                </a>
              </div>
            </motion.div>
          ...
          <div className="bg-white rounded-[4rem] p-12 md:p-20 shadow-xl border border-slate-100">
            <div className="flex flex-col lg:flex-row gap-16">
              <div className="lg:w-1/2">
                ...
                <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-8">{t.educationalProgram.title}</h2>
                <div className="space-y-6">
                  <h4 className="text-xl font-black flex items-center gap-3 text-[#0056b3]"><Target size={24} /> {t.educationalProgram.objectivesTitle}</h4>
                  <ul className="grid gap-4">
                    {t.educationalProgram.objectives.map((obj, i) => (
                      <li key={i} className="flex gap-3 text-slate-600 font-medium leading-relaxed">
                        <div className="mt-1.5 shrink-0 w-2 h-2 rounded-full bg-[#0056b3]" /> {obj}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <div className="lg:w-1/2 space-y-12">
                ...
                <div className="bg-[#0056b3] p-10 rounded-[2.5rem] text-white shadow-xl shadow-[#0056b3]/20">
                  ...
                  <ul className="space-y-4">
                    {t.educationalProgram.offers.map((offer, i) => (
                      <li key={i} className="flex gap-3 font-bold text-blue-50">
                        <CheckCircle2 size={20} className="shrink-0 text-[#00a8e8]" /> {offer}
                      </li>
                    ))}
                  </ul>
                </div>
      ...
      <section id="about" className="section-padding bg-white">
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-20 items-center mb-24">
            <div>
              <div className="text-[#0056b3] font-black uppercase tracking-[0.3em] text-sm mb-4">{lang === 'en' ? 'Our Story' : 'हाम्रो कथा'}</div>
              <h2 className="text-4xl md:text-6xl font-black text-slate-900 mb-10">{t.about.title}</h2>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="p-8 rounded-3xl bg-slate-50 border border-slate-100">
                  <h4 className="text-xl font-black mb-4 text-[#0056b3]">{t.about.visionTitle}</h4>
                  <p className="text-slate-600 font-medium">{t.about.vision}</p>
                </div>
                <div className="p-8 rounded-3xl bg-[#0056b3] text-white shadow-xl shadow-[#0056b3]/20">
              ...
              <div className="absolute -bottom-10 -left-10 bg-white p-10 rounded-[2.5rem] shadow-2xl border border-slate-100 max-w-sm">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm font-bold text-[#0056b3] uppercase tracking-widest">{t.about.mdRole}</div>
                  </div>
                </div>
                <p className="text-sm text-slate-600 italic font-medium leading-relaxed">&ldquo;{t.about.mdMessage}&rdquo;</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="section-padding bg-slate-900 text-white">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto mb-20">
            <h2 className="text-4xl md:text-6xl font-black mb-8">{t.whyChooseUs.title}</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className="p-8 rounded-3xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-[#00a8e8] transition-all group">
                <div className="w-14 h-14 bg-[#0056b3]/20 text-[#00a8e8] rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">{item.icon}</div>
      ...
      <section className="section-padding bg-white">
        <div className="container mx-auto px-4">
          <div className="bg-[#0056b3] rounded-[4rem] p-12 md:p-24 overflow-hidden relative text-white">
            <div className="absolute top-0 right-0 w-96 h-96 bg-white/10 rounded-full blur-[100px]" />
            <div className="relative z-10 text-center mb-16">
              <h2 className="text-4xl md:text-6xl font-black mb-6">{t.branches.title}</h2>
              <p className="text-xl text-blue-100 font-medium">{t.branches.subtitle}</p>
            </div>
            <div className="flex flex-wrap justify-center gap-4 relative z-10">
              {t.branches.locations.map((loc, i) => (
                <motion.div key={i} whileHover={{ scale: 1.05 }} className="px-10 py-5 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 font-black text-xl hover:bg-white hover:text-[#0056b3] transition-all cursor-default">{loc}</motion.div>
              ))}
            </div>
          </div>
        </div>
      ...
      <section id="contact" className="section-padding bg-slate-50">
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-20">
            <div>
              <div className="text-[#0056b3] font-black uppercase tracking-[0.3em] text-sm mb-4">{lang === 'en' ? 'Contact Us' : 'सम्पर्क'}</div>
              <h2 className="text-4xl md:text-6xl font-black text-slate-900 mb-8">{t.contact.title}</h2>
              <p className="text-xl text-slate-600 mb-12 font-medium">{t.contact.subtitle}</p>
              <div className="space-y-10">
                <div className="flex gap-8">
                  <div className="w-16 h-16 bg-white text-[#0056b3] rounded-2xl flex items-center justify-center shadow-lg shrink-0"><MapPin size={32} /></div>
                  <div>
                ...
                <div className="flex gap-8">
                  <div className="w-16 h-16 bg-white text-[#0056b3] rounded-2xl flex items-center justify-center shadow-lg shrink-0"><Phone size={32} /></div>
                  <div>
                ...
                <div className="flex gap-8">
                  <div className="w-16 h-16 bg-white text-[#0056b3] rounded-2xl flex items-center justify-center shadow-lg shrink-0"><Mail size={32} /></div>
                  <div>
              ...
              <form className="space-y-8" onSubmit={(e) => e.preventDefault()}>
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    ...
                    <input type="text" className="w-full px-6 py-4 rounded-2xl bg-slate-50 border-2 border-transparent focus:border-[#0056b3] focus:bg-white outline-none transition-all font-bold" placeholder={lang === 'en' ? 'Your Name' : 'तपाईंको नाम'} />
                  </div>
                  <div className="space-y-3">
                    <label className="text-sm font-black text-slate-700 uppercase tracking-widest">{t.contact.form.phone}</label>
                    <input type="tel" className="w-full px-6 py-4 rounded-2xl bg-slate-50 border-2 border-transparent focus:border-[#0056b3] focus:bg-white outline-none transition-all font-bold" placeholder={lang === 'en' ? 'Phone Number' : 'फोन नम्बर'} />
                  </div>
                </div>
                <div className="space-y-3">
                  <label className="text-sm font-black text-slate-700 uppercase tracking-widest">{t.contact.form.message}</label>
                  <textarea rows={5} className="w-full px-6 py-4 rounded-2xl bg-slate-50 border-2 border-transparent focus:border-[#0056b3] focus:bg-white outline-none transition-all font-bold" placeholder={lang === 'en' ? 'How can we help you?' : 'हामीले कसरी सहयोग गर्न सक्छौं?'} />
                </div>
                <button className="w-full bg-[#0056b3] text-white py-5 rounded-2xl font-black text-xl shadow-2xl shadow-[#0056b3]/30 hover:bg-blue-700 transition-all hover:-translate-y-1">{t.contact.form.submit}</button>
              </form>
            </div>
          </div>
        </div>
          ...
          <div className="grid lg:grid-cols-4 gap-16 mb-20">
            <div className="lg:col-span-2">
              <div className="flex items-center gap-4 mb-8">
                ...
                <span className="font-black text-3xl">{lang === 'en' ? 'All Nepal Health' : 'अल नेपाल हेल्थ'}</span>
              </div>
            ...
            <div>
              ...
              <ul className="space-y-5 text-lg font-bold text-slate-400">
                {navItems.map(item => (
                  <li key={item.name}><a href={item.href} className="hover:text-[#00a8e8] transition-colors">{item.name}</a></li>
                ))}
              </ul>
            </div>
            <div>
              ...
              <ul className="space-y-5 text-lg font-bold text-slate-400">
                ...
                <li className="flex justify-between text-[#00a8e8] font-black"><span>{t.footer.emergency.split(':')[0]}</span> <span>{t.footer.emergency.split(':')[1]}</span></li>
              </ul>
            </div>
          </div>
